...
class FormNursePage extends StatefulWidget {
  const FormNursePage({
    Key? key,
    this.nurse,
  }) : super(key: key);
  ...
  @override
  State<FormNursePage> createState() => _FormNursePageState();
}

class _FormNursePageState extends State<FormNursePage> {
  final _formKey = GlobalKey<FormState>();

  NurseModel _nurse = NurseModel();
  ...
  _onUpdateOrCreate() {
    if (widget.nurse != null) {
      _nurse = widget.nurse ?? NurseModel();
      _nameCtrl?.text = widget.nurse?.name ?? "";
      _emailCtrl?.text = widget.nurse?.email ?? "";
      _nipCtrl?.text = widget.nurse?.nip ?? "";
      _sipCtrl?.text = widget.nurse?.sip ?? "";
      _polyCtrl?.text = widget.nurse?.polyclinic?.name ?? "";
      _addressCtrl?.text = widget.nurse?.address ?? "";
      _dobCtrl?.text = widget.nurse?.dob?.toString() ?? "";
      _genderCtrl?.text =
          Helper.getKeyOrValueMapGender(widget.nurse?.gender, false);
      _selectedPolyclinic = widget.nurse?.polyclinic;
    }
  }

  _onCreate(NurseViewModel value) async {
    if (_formKey.currentState!.validate()) {
      _formKey.currentState!.save();
      await value.createNurse(_nurse).then((ress) {
  ...
  _onUpdate(NurseViewModel value) async {
    if (_formKey.currentState!.validate()) {
      _formKey.currentState!.save();
      await value.updateNurse(_nurse).then((ress) {
        if (value.state == RequestState.LOADED) {
          Navigator.pop(context);
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text("Tambah Perawat"),
        ),
              ...
              child: Column(
                children: [
                  ...
                  DropdowndSearchWidget(
                    ...
                    label: "Jenis Kelamin",
                    onChanged: (value) {
                      _nurse.gender = Helper.getKeyOrValueMapGender(value);
                    },
                  ),
                  const SizedBox(height: 12),
                  ...
                  const SizedBox(height: 12),
                  Consumer<NurseViewModel>(builder: (context, value, _) {
                    ...
                    return SizedBox(
                      ...
                      child: ElevatedButton(
                        ...
                        child: Text(
                          widget.nurse != null ? "Update" : "Daftar",
                          style: kBodyText.copyWith(
                            color: Colors.white,
                            fontSize: 16,
